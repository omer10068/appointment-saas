...
11. **Onboarding summary — admin/ops read-only:** Platform admins can retrieve a compound onboarding snapshot for any business regardless of status via `GET /admin/businesses/:businessId/onboarding-summary`. Returns business metadata, all businessUsers (with user phone/email), services, serviceProviders (with `serviceIds[]` and `hasWorkingHours`), businessWorkingHours, and an embedded `readiness` block. Read-only — no mutations. Does not replace `GET /admin/businesses/:businessId/readiness`, which remains the lean activation gate used by `setBusinessStatus` and `setPublicBookingEnabled`.

12. **Business metadata correction — admin/ops during any status:** Platform admins can update a business's `name`, `timezone`, `locale`, and `currency` via `PATCH /admin/businesses/:businessId`. Works for DRAFT, TRIAL, and ACTIVE businesses. At least one field is required — empty body returns 400. `slug` is intentionally immutable and is not in `UpdateBusinessSettingsDto`; sending it returns 400 (`forbidNonWhitelisted`). `publicBookingEnabled` is not changed here (separate endpoint). Dashboard business-settings behavior unchanged.

13. **Service field correction — admin/ops during any status:** Platform admins can update any service field (`name`, `description`, `durationMinutes`, `priceCents`, `bufferBeforeMin`, `bufferAfterMin`, `isActive`) via `PATCH /admin/businesses/:businessId/services/:serviceId`. Works for DRAFT, TRIAL, and ACTIVE businesses. Verifies service belongs to `businessId` → 404 if cross-tenant. No cascade behavior when deactivating a service — if an active SP is linked to the deactivated service, the readiness check will reflect this (`allActiveProvidersHaveActiveServiceAssignment: false`). Dashboard service-update behavior unchanged.

14. **ServiceProvider correction — admin/ops during any status:** Platform admins can update a ServiceProvider's `displayName`, `serviceIds`, and `isActive` via `PATCH /admin/businesses/:businessId/service-providers/:serviceProviderId`. Works for DRAFT, TRIAL, and ACTIVE businesses. Verifies SP belongs to `businessId` → 404 if cross-tenant. Preserves all creation-time invariants: active SP cannot link inactive services → 400; activating an SP with no existing service assignments → 400; `serviceIds: []` is rejected by DTO `@ArrayMinSize(1)`. `businessUserId` is intentionally immutable and absent from `UpdateServiceProviderDto`. When `serviceIds` is provided, all existing service links are deleted and replaced in a transaction. Dashboard SP-update behavior unchanged.
